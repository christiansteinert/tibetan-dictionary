<?php
$ZLIB_DICT = " perfections pupil ru. skom Sumeru summary treat tshi vidyadhara voidness (1998) adorned battle bright, ". 
...
"be chen lta is mi 1) ba'i par 2) gi gyi po nas du ma dang in ba kyi la and to pa'i pa the of ";
  ...
  function inflate($bytes) {
    global $ZLIB_DICT;
    $context = inflate_init(ZLIB_ENCODING_RAW, [ 'dictionary' => $ZLIB_DICT ] );
    return inflate_add($context, $bytes, ZLIB_FINISH);
  }
  ...
  if(isset($_POST['term'])) { // search for the definition of a term
    ...
    if( strpos($search, '*') !== false ) { // wildcard search
        ...
        $statement = $db->prepare('SELECT DISTINCT term FROM '.$table.' inner join DICTNAMES on '.$table.'.dictionary = DICTNAMES.id and DICTNAMES.language = "'.$lang.'" WHERE ((( '.$table.'.term = :word COLLATE NOCASE ) OR ( '.$table.'.term > :wordSearch1 COLLATE NOCASE AND term < :wordSearch2 COLLATE NOCASE )) AND ('.$dictQuery.')) GROUP BY term ORDER BY lower(term), term LIMIT '.$maxresults.' OFFSET '.$offset.';');
