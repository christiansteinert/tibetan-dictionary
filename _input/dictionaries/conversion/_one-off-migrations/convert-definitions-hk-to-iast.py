#!/usr/bin/env python3
"""
Convert Harvard-Kyoto (HK) transliteration to IAST in CSV definitions.

This script reads pipe-separated CSV files (two columns: headword|definition),
converts the second column (definition) from Harvard-Kyoto transliteration to IAST
...
Usage:
    python convert-definitions-hk-to-iast.py <file1.csv> [file2.csv ...]

Example:
    python convert-definitions-hk-to-iast.py mydict.csv
    python convert-definitions-hk-to-iast.py *.csv
"""
...
def convert_file(filepath):
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if "|" in stripped:
            headword, definition = stripped.split("|", 1)
            iast_definition = converter.inter_transliterate("sen", "hk", "iast", sentence=definition)
            converted_lines.append(f"{headword}|{iast_definition}\n")
        else:
