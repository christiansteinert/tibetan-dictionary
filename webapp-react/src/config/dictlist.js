export const GROUPED_DICTLIST =
{
  ...
  "MastersProgramGlossary": {
    label: 'FPMT Masters Program Glossary for Abhisamaya&shy;alamkara',
  ...
  "_Hopkins-Tibetan": {
    type: 'group',
    label: 'Hopkins Tibetan 2015',
    about: 'Tibetan Definitions, synonyms and examples by Jeffrey Hopkins et al|Source: The Uma Institute for Tibetan Studies Tibetan-Sanskrit-English Dictionary (Version: June 2015)|Jeffrey Hopkins, Editor.|Paul Hackett, Contributor and Technical Editor.| Contributors: Nathaniel Garson, William Magee, Andres Montano, John Powers, Craig Preston, Joe Wilson, Jongbok Yi|A PDF version of this dictionary is available for download at: www.uma-tibet.org',
    public: "true",
    language: ["tib"],
    items: {
...
for (const [groupKey, groupValue] of Object.entries(GROUPED_DICTLIST)) {
  if (groupValue.type === 'group') {
    for (const [itemKey, itemValue] of Object.entries(groupValue.items)) {
      DICTLIST[itemKey] = itemValue;
      itemValue.groupId = groupKey; // remember the group this item belongs to
    }
  } else {
    DICTLIST[groupKey] = groupValue;
  }
}