...
import io.sqlc.custom.DictDecompress;
                ...
                try {
                    for (int i = 0; i < colCount; ++i) {
                        ...
                        case SQLColumnType.BLOB:
                            // BEGIN MODIFICATION FOR TIBETAN DICTIONARY
                            // Use getColumnTextNativeString as a fallback or check if the library provides a way to get bytes
                            // Since the compile error shows getColumnBlob(int) doesn't exist, we'll use the native string method
                            // which for BLOBs in liteglue usually returns the bytes as a string or base64
                            row.put(key, DictDecompress.inflateText(myStatement.getColumnTextNativeString(i).getBytes(StandardCharsets.UTF_8)));
                            break;
                            // END MODIFICATION FOR TIBETAN DICTIONARY
